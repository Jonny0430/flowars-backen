...
export class ProductService {
	...
	public async createProduct(dto: PetProductDto, _id: string) {
		...
		await this.agentModel.findOneAndUpdate({ author: _id }, { $push: { products: product._id } }, { new: true })
		return 'Success'
	}
	...
	public async deleteProduct(productId: string, userId: string) {
		...
		await this.agentModel.findOneAndUpdate({ author: userId }, { $pull: { products: productId } }, { new: true })
	...
	async getDetailedProduct(slug: string) {
		const product = (await this.productModel.findOne({ slug }).populate('author').exec()) as ProductDocument & {
			reviewCount: number
			reviewAvg: number
		}
